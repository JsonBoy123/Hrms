...
use App\Models\Employees\LeaveRuleHistory;
use App\Models\Employees\LeaveRuleEmployees;
...
class LeavesController extends Controller
{
  ...
  //}
    ...
    public function tl_approval(Request $request, $request_id){
        ...
        if($app_request->teamlead_approval == 0){
            ...
            if($request->action == 1){
                ...
                add_log_activities('Leave approved by TL and leave apply id is '.$request_id,'hrms');
        ...
        if($app_request->subadmin_approval == 0){
            ...
            if($request->action == 1){
                ...
                LeaveApply::where('id',$request_id)->update(['subadmin_approval' => $request->action]);
                 ...
                 add_log_activities('Leave approved by HR and leave apply id is '.$request_id,'hrms');
        ...
        if($app_request->admin_approval == 0){
            ...
            if($request->action == 1){
                ...
                LeaveApply::where('id',$request_id)->update(['admin_approval' => $request->action]);
                 ...
                 add_log_activities('Leave approved by ADMIN and leave apply id is '.$request_id,'hrms');
                //Update record when application approve
    ...
    * 
    */
    #Update leaves Balance by Scheduling
    ...
    public function indexRules(){
        ...
        $leaves = LeaveMast::first();

        //$rules = EmployeeMast::where('user_id', Auth::id())->first();

        // $rule_emp = LeaveRuleEmployees::with('employees')
        //                 ->get();

        //$users = User::wherePermissionIs(['hrms-manage-leave-rule'])->get();

        // dd($users);

        return view('HRD.leaves.rules.index', compact('leaves' ));
        
    }

    public function activateRules(Request $request){

		//return $request->all();
        LeaveMast::where('id', '>', 0)
                ->update(['status' => $request->leavestatus]);

        LeaveRuleHistory::create([
        	'user_id'	=> Auth::id(),
        	'action'	=> $request->leavestatus
        ]);

        return back()->with('success', 'Status has been updated.');
    }

    public function RulesEmployees(Request $request){

        $employees = EmployeeMast::all();

        $leaveEmp = LeaveRuleEmployees::all();

        return view('HRD.leaves.rules.create', compact('employees', 'leaveEmp'));
    }

    public function leaveAllotmenttest(){

        $leavemast = LeaveMast::all();
        $leaveallot = LeaveAllotment::paginate();
        
        
    }

    /*public function storeRulesEmployees(Request $request){

    	return $request->all();
        $user = LeaveRuleEmployees::where('user_id', $request->user_id)->first();

        if($user == null){
            LeaveRuleEmployees::create([
            ...
            $emp = LeaveRuleEmployees::where('user_id', $request->user_id)->first();

            $flag = $emp->see_employees == 1 ? 0 : 1;

            LeaveRuleEmployees::where('user_id', $request->user_id)
                ->update(['see_employees' => $flag]);
        }
    ...
    public function destroyRulesEmployees( $id){

        EmployeeMast::where('user_id', $id)
            ->update(['update_leaverule' => 0]);

        LeaveRuleEmployees::where('user_id', $id)->delete();
