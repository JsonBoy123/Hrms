              ...
              @if(!empty($balance['allotments']))
                @if(count($balance['allotments']) != 0)
                  @if($balance['allotments'][0]->status == 1)
                    <span class="ml-2">
                      <a href="{{url('employee/leaves/create')}}" class="btn btn-sm btn-success">
                      <span class="fa fa-plus "></span> Apply for Leave</a>
                    </span>
                  @endif
                @endif
              @endif
              <a href="{{ URL::previous() }}" class="btn btn-sm btn-primary pull-right" style="{background-color: #e7e7e7; color: black;}" >Back</a></h1>
          </h1>
...
--}}
								...
								<td>
									...
									@elseif($leaveapply->admin_approval == 1)
										<div class="text-center">
										 	<strong style="color: green;" class="text-center">
										 		APPROVE
											</strong>
										</div>
								...
								@if(in_array($leaveapply->teamlead_approval, [0, 4]) && in_array($leaveapply->subadmin_approval, [0]))
									{{-- <div class="ml-2 ">
