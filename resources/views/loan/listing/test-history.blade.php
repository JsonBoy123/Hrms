		...
		<div class="row col-12">
			...
			<div class="col-6 form-group">
				...
				@else
					...
					{{$emi_credit->emi_remain == 0 ? round($request->emi_alloted) : $emi_credit->emi_remain}}
				@endif
			</div>
			<div class="col-6 form-group">
				<b>Loan Payment : </b>
				<a href="{{route('test-emi.update', $request->id)}}" class="btn btn-sm btn-primary "  style="{background-color: #e7e7e7; color: black;}" >Update</a>
			</div>
		</div><br>
		...
		<form action="{{route('test-history.store', $request->id)}}" method="POST">
			...
			<div class="row">
				...
				<div class="col-6 form-group">
					...
					<input type="text" class="form-control datepicker" name="deduction_date" value="{{!empty($emi_credit)}}" autocomplete="off">
			...
			<div class="row"> --}}
				...
				if(empty($emi_credit)){
					...
					$credit = $emi_credit->emi_remain == 0 ? '' : $emi_credit->emi_remain;
				} ?>
				
				<div class="col-6 form-group" style="float: right">
					<input type="hidden" class="form-control contact" id="next_emi" name="next_emi" value="{{ $credit }}" readonly="">
				</div>
			</div>
			<div class="row">
		...
		<table class="table table-striped table-hover table-bordered" id="CandidatesTable">
				...
				@foreach($history as $index)
					<tr class="text-center">
						...
						<td>{{round($index->emi_remain)}} Rs.</td>
						@ability('hrms_admin|hrms_hr', 'Hrms-manage-loan-request')
						<td>
