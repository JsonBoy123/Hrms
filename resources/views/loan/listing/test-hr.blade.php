          ...
          @php $count = 0; @endphp
           @foreach($requests as $index)
              <tr class="text-center" >
                ...
                <td>{{$index->requested_amount}}</td>
                <td>{{$index->reason}}</td>
                <td>
                  @if($index->accountant == 1 && $index->admin == 1)
                    <strong style="color: #0cac0c;">DISBURSED</strong>
                  @elseif($index->admin == 1)
               ...
               {{--  <td>
                ...
                <td>
                 @if($index->subadmin == 0)
                    <strong style="color: grey;">PENDING</strong>
                  @elseif($index->subadmin == 1)
                    <strong style="color: #0cac0c;">APPROVED</strong>
                  @elseif($index->subadmin == 2)
                    <strong style="color: #3375ca;">DECLINED</strong>
                  @elseif($index->subadmin == 3)
                    <strong class="dec_msg">DISBURSED</strong>
                  @endif
                </td>
                <td>
                  ...
                  @elseif($index->admin == 1)
                    <strong style="color: #0cac0c;">APPROVED</strong>
                  @elseif($index->admin == 2)
                    <strong style="color: #3375ca;">DECLINED</strong>
                  @elseif($index->admin == 3)
                    <strong class="dec_msg">DISBURSED</strong>
                  @endif
                </td>
                {{-- @ability('hrms_admin', 'hrms-edit') --}}
                ...
                <td class='text-center' style="border-bottom:none">
                  ...
                  @elseif($index->accountant == 1)

                    <span class="ml-2">
                      <a href="{{route('test-history',$index->id)}}" class="btn btn-sm btn-info"><i class="fa fa-flickr" aria-hidden="true"></i> History</a>
                    </span>
