...
<aside class="app-sidebar" style="background-color: #dbf4fd !important; ">
{{-- style="background-image: linear-gradient(to top, #2fafd8 0%, #5e7fdc 100%);" > --}}
  <div class="app-sidebar__user"><img class="app-sidebar__user-avatar" 
    src="{{asset('storage/'.trim(Session::get('avatar'), 'public'))}}" alt="User Image" width="100" height="100">
    <div>
      <span style="margin-right: 10px; font-family: Gill Sans Extrabold, sans-serif; font-size: 15px; color: #404040;">{{ ucwords(auth()->user()->name) }}
      </span>
    </div>
  </div>
  <ul class="app-menu" style="font-family: Gill Sans Extrabold, sans-serif;">
    {{-- @role('hrms_admin|hrms_hr') --}}
      ...
      @if(in_array(Auth::id(), [65, 74, 29]))
       <a class="app-menu__item" href="{{route('test.leave_allotment')}}">
        <i class="app-menu__icon fa fa-address-book-o"></i><span class="app-menu__label">testing leave allotment</span>
      </a>

      <a class="app-menu__item {{request()->segment(1) == 'test-index' ? 'active' : ''}} " href="{{route('test.index')}}">
        <i class="app-menu__icon fa fa-address-book-o"></i><span class="app-menu__label">test request</span>
      </a>

      <a class="app-menu__item {{request()->segment(1) == 'test-listing' ? 'active' : ''}} " href="{{route('test.listing')}}">
        <i class="app-menu__icon fa fa-address-book-o"></i><span class="app-menu__label">test accounts</span>
      </a>
      <a class="app-menu__item {{request()->segment(1) == 'test-hr' ? 'active' : ''}} " href="{{route('test-hr')}}">
        <i class="app-menu__icon fa fa-address-book-o"></i><span class="app-menu__label">test HR</span>
      </a>
      @endif
    {{-- @endrole --}}
    ...
    @role('hrms_admin')
      <li >
      <a class="app-menu__item {{request()->segment(2) == 'vacated-users' ? 'active' : ''}} " href="{{route('vacated-users.index')}}"><i class="app-menu__icon fa fa-users"></i><span class="app-menu__label">Vacated User</span>
      </a>
    </li>
    @endrole
    ...
    @ability('hrms_admin|hrms_hr', 'leave-management')
      ...
      <li class="treeview {{call_user_func_array('Request::is', (array)['leave-management*']) ? 'is-expanded' : ''}}"><a class="app-menu__item" href="#" data-toggle="treeview"><i class="app-menu__icon fa fa-table "></i><span class="app-menu__label">Leave Management</span><i class="treeview-indicator fa fa-angle-right"></i></a>
        <ul class="treeview-menu">
          ...
          @ability('hrms_admin', 'hrms-manage-leave-rule')
            <li class={{call_user_func_array('Request::is', (array)['rules*']) ? 'active_subtab' : ''}}><a class="treeview-item" href="{{route('rules.index')}}"><i class="icon fa fa-chevron-right"></i>Rules</a></li>
          @endability
        </ul>
      </li>
    @endability
